...
class Sample(Semiring):
    """Lazy sampling semiring: Sample(w, d) carries weight w and derivation d.

    `sample()` draws a (weight, derivation) pair; iteration yields independent
    samples forever. Addition mixes with weights proportional to `w`;
    multiplication samples both children independently and combines.
    """
...
class Star(Sample):
    ...
    def sample(self):
        # geometric-distribution-style repetition
        zs = 1
        zd = []
        while True:
