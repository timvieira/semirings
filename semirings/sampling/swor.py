import numpy as np

from semirings.base import Semiring


class Sample(Semiring):
    """Sampling-without-replacement semiring.

    `sample()` returns a LazySort element carrying a fresh exponential variate
    tied to the derivation. Summed / multiplied Samples combine their sampled
    LazySorts.
    """

    def __init__(self, w, d):
        self.w = w
        self.d = d

    def sample(self):
        from semirings import LazySort
        X = -np.log(np.random.uniform(0, 1)) / self.w
        return LazySort(X, self.d)

    def __add__(self, other):
        if other is zero: return self
        if self is zero: return other
        return Sum(self, other)

    def __mul__(self, other):
        if other is one: return self
        if self is one: return other
        if other is zero: return zero
        if self is zero: return zero
        return Prod(self, other)

    def __repr__(self):
        return f'{self.__class__.__name__}({self.w}, {self.d})'


class Sum(Sample):
    def __init__(self, x, y):
        self.x = x
        self.y = y
        super().__init__(x.w + y.w, d=[x, y])

    def sample(self):
        return self.x.sample() + self.y.sample()


class Prod(Sample):
    def __init__(self, x, y):
        self.x = x
        self.y = y
        super().__init__(x.w * y.w, d=[x, y])

    def sample(self):
        return self.x.sample() * self.y.sample()


zero = Sample(0.0, None)
one = Sample(1.0, ())

Sample.one = one
Sample.zero = zero
